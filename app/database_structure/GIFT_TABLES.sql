--Gift Details View for SELECT--
create or replace view MY_FLOW.PRESENTATION.VIEW_GIFT_DETAILS(
	GIFT_ID,
	MERCHANT_NAME,
	KAM_NAME,
	KAM_EMAIL,
	CREATED_DATE,
	WORKFLOW_STATUS,
	MEMBER_ID,
	MEMBER_LOGIN,
	FULL_NAME,
	PHONE,
	ADDRESS,
	REWARD_NAME,
	GIFT_ITEM,
	GIFT_COST,
	CURRENCY,
	DESCRIPTION,
	REWARD_CLUB_ORDER,
	CATEGORY,
	MANAGER_NAME,
	MANAGER_EMAIL,
	REJECT_REASON,
	DISPATCHER,
	TRACKING_CODE,
	TRACKING_STATUS,
	MKTOPS_NAME,
	MKTOPS_EMAIL,
	MKT_PURCHASE_DATE,
	UPLOADED_BO,
	MKT_PROOF,
	KAM_PROOF,
	KAM_PROOF_NAME,
	KAM_PROOF_EMAIL,
	GIFT_FEEDBACK,
	AUDITER_NAME,
	AUDITER_EMAIL,
	AUDIT_DATE,
	AUDIT_REMARK,
	LAST_MODIFIED_DATE
) as
SELECT
    GD.GIFT_ID,
    REF.DESCRIPTION        AS MERCHANT_NAME,
    U.NAME                 AS KAM_NAME,
    U.EMAIL                AS KAM_EMAIL,
    DATEADD('hour', 8, GD.CREATED_DATE) AS CREATED_DATE,
    GD.WORKFLOW_STATUS,
    MP.MEMBER_ID,
    MP.MEMBER_LOGIN,
    MP.MEMBER_NAME         AS FULL_NAME,
    NULL AS PHONE,
    NULL AS ADDRESS,
    GD.REWARD_NAME,
    GD.GIFT_ITEM,
    GD.GIFT_COST,
    GD.CURRENCY,
    GD.DESCRIPTION,
    GD.REWARD_CLUB_ORDER,
    GD.CATEGORY,
    U2.NAME                AS MANAGER_NAME,
    U2.EMAIL               AS MANAGER_EMAIL,
    GD.REJECT_REASON,
    GD.DISPATCHER,
    GD.TRACKING_CODE,
    GD.TRACKING_STATUS,
    U3.NAME                AS MKTOPS_NAME,
    U3.EMAIL               AS MKTOPS_EMAIL,
    GD.MKT_PURCHASE_DATE,
    GD.UPLOADED_BO,
    GD.MKT_PROOF,
    GD.KAM_PROOF,
    U4.NAME                AS KAM_PROOF_NAME,
    U4.EMAIL               AS KAM_PROOF_EMAIL,
    GD.GIFT_FEEDBACK,
    U5.NAME                AS AUDITER_NAME,
    U5.EMAIL               AS AUDITER_EMAIL,
    GD.AUDIT_DATE,
    GD.AUDIT_REMARK,
    DATEADD('hour', 8, GD.LAST_MODIFIED_DATE) AS LAST_MODIFIED_DATE
FROM MY_FLOW.PUBLIC.GIFT_DETAILS GD
LEFT JOIN MY_FLOW.MART.ALL_MEMBER_PROFILE MP
    ON GD.MEMBER_ID = MP.MEMBER_ID
LEFT JOIN MY_FLOW.GLOBAL_CONFIG.SYS_USER_INFO U
    ON GD.KAM_REQUESTED_BY = U.USER_ID
LEFT JOIN MY_FLOW.GLOBAL_CONFIG.SYS_USER_INFO U2
    ON GD.APPROVAL_REVIEWED_BY = U2.USER_ID
LEFT JOIN MY_FLOW.GLOBAL_CONFIG.SYS_USER_INFO U3
    ON GD.PURCHASED_BY = U3.USER_ID
LEFT JOIN MY_FLOW.GLOBAL_CONFIG.SYS_USER_INFO U4
    ON GD.KAM_PROOF_BY = U4.USER_ID
LEFT JOIN MY_FLOW.GLOBAL_CONFIG.SYS_USER_INFO U5
    ON GD.AUDITED_BY = U5.USER_ID
LEFT JOIN MY_FLOW.GLOBAL_CONFIG.REFERENCE_TABLE REF
    ON MP.MERCHANT_ID = REF.CODE
WHERE GD.BATCH_ID IS NULL
   OR GD.BATCH_ID IN (
        SELECT BATCH_ID
        FROM MY_FLOW.PUBLIC.BULK_IMPORT_BATCHES
        WHERE IS_ACTIVE = TRUE
    );

-- Create & Update any changes to the gift tables here
create or replace TABLE MY_FLOW.PUBLIC.GIFT_DETAILS (
	GIFT_ID NUMBER(38,0) NOT NULL autoincrement start 100 increment 1 order,
	MEMBER_ID NUMBER(38,0),
	BATCH_ID NUMBER(38,0),
	KAM_REQUESTED_BY VARCHAR(16777216),
	CREATED_DATE TIMESTAMP_NTZ(9),
	WORKFLOW_STATUS VARCHAR(16777216),
	MEMBER_LOGIN VARCHAR(16777216),
	REWARD_NAME VARCHAR(16777216),
	GIFT_ITEM VARCHAR(16777216),
	GIFT_COST NUMBER(38,0),
	CURRENCY VARCHAR(16777216),
	DESCRIPTION VARCHAR(16777216),
	REWARD_CLUB_ORDER VARCHAR(16777216),
	CATEGORY VARCHAR(16777216),
	APPROVAL_REVIEWED_BY VARCHAR(16777216),
	DISPATCHER VARCHAR(16777216),
	TRACKING_CODE VARCHAR(16777216),
	TRACKING_STATUS VARCHAR(16777216),
	PURCHASED_BY VARCHAR(16777216),
	MKT_PURCHASE_DATE TIMESTAMP_NTZ(9),
	UPLOADED_BO BOOLEAN,
	MKT_PROOF VARCHAR(16777216),
	KAM_PROOF VARCHAR(16777216),
	KAM_PROOF_BY VARCHAR(16777216),
	GIFT_FEEDBACK VARCHAR(16777216),
	AUDITED_BY VARCHAR(16777216),
	AUDIT_DATE TIMESTAMP_NTZ(9),
	AUDIT_REMARK VARCHAR(16777216),
	LAST_MODIFIED_DATE TIMESTAMP_NTZ(9),
	REJECT_REASON VARCHAR(16777216),
	MKT_DELIVERED_DATE TIMESTAMP_NTZ(9),
	primary key (GIFT_ID)
);

--Gift Workflow Timeline
create or replace TABLE MY_FLOW.PUBLIC.GIFT_WORKFLOW_TIMELINE (
	ID NUMBER(38,0) NOT NULL autoincrement start 100 increment 1 order,
	GIFT_ID NUMBER(38,0),
	FROM_STATUS VARCHAR(16777216),
	TO_STATUS VARCHAR(16777216),
	CHANGED_BY NUMBER(38,0),
	CHANGED_AT TIMESTAMP_NTZ(9),
	REMARK VARCHAR(16777216),
	primary key (ID)
);
-- Gift Workflow Timeline View
create or replace view MY_FLOW.PRESENTATION.VIEW_GIFT_WORKFLOW_TIMELINE(
	WORKFLOW_ID,
	GIFT_ID,
	TIMELINE_TITLE,
	UPDATE_DATE,
	UPDATE_TIME,
	REMARK
) as SELECT 
    GWT.ID,
    GWT.GIFT_ID,
    
    CASE
        WHEN GWT.FROM_STATUS IS NULL AND GWT.TO_STATUS = 'KAM_Request'
        THEN 'Gift Request Submitted'

        WHEN GWT.FROM_STATUS = 'KAM_Request' AND GWT.TO_STATUS = 'Manager_Review'
        THEN 'Manager Review'
        
        WHEN GWT.FROM_STATUS = 'Manager_Review' AND GWT.TO_STATUS = 'MKTOps_Processing'
        THEN 'MKTOps Processing'
        
        WHEN GWT.FROM_STATUS = 'MKTOps_Processing' AND GWT.TO_STATUS = 'KAM_Proof'
        THEN 'Delivery Confirmation'
        
        WHEN GWT.FROM_STATUS = 'KAM_Proof' AND GWT.TO_STATUS = 'SalesOps Audit'
        THEN 'SalesOps Audit'

        WHEN GWT.FROM_STATUS = 'SalesOps_Audit' AND GWT.TO_STATUS = 'KAM_Proof'
        THEN 'Audit Issue'
        
        WHEN GWT.FROM_STATUS = 'SalesOps_Audit' AND GWT.TO_STATUS = 'Completed'
        THEN 'Completed'
        
        WHEN GWT.FROM_STATUS = 'KAM_Proof' AND GWT.TO_STATUS = 'MKTOps_Processing'
        THEN 'Reverted to MKTOps'
        
        ELSE NULL
        
    END AS TIMELINE_TITLE,
    
    TO_VARCHAR(DATEADD('hour', 8, CHANGED_AT), 'YYYY-MM-DD') AS UPDATE_DATE,
    TO_VARCHAR(DATEADD('hour', 8, CHANGED_AT), 'HH12:MIAM') AS UPDATE_TIME,
    
    CASE
        WHEN GWT.FROM_STATUS IS NULL AND GWT.TO_STATUS = 'KAM_Request' 
        THEN 'Request submitted by ' || GD.KAM_NAME || ' for ' || GD.FULL_NAME
        
        WHEN GWT.FROM_STATUS = 'KAM_Request' AND GWT.TO_STATUS = 'Manager_Review' AND GD.REJECT_REASON IS NULL
        THEN 'Pending Manager Approval'

        WHEN GWT.FROM_STATUS = 'KAM_Request' AND GWT.TO_STATUS = 'Manager_Review' AND GD.REJECT_REASON IS NOT NULL
        THEN 'Rejected by ' || U.NAME || ': ' || GD.REJECT_REASON

        WHEN GWT.FROM_STATUS = 'Manager_Review' AND GWT.TO_STATUS = 'MKTOps_Processing'
        THEN 'Pending parcel to be picked up by dispatcher'

        WHEN GWT.FROM_STATUS = 'Manager_Review' AND GWT.TO_STATUS = 'MKTOps_Processing' AND GD.TRACKING_STATUS = 'In Transit'
        THEN 'Parcel out for delivery via ' || GD.DISPATCHER || '. Tracking Code: ' || GD.TRACKING_CODE

        WHEN GWT.FROM_STATUS = 'Manager_Review' AND GWT.TO_STATUS = 'MKTOps_Processing' AND GD.TRACKING_STATUS = 'Delivered'
        THEN 'Parcel successfully delivered'

        WHEN GWT.FROM_STATUS = 'Manager_Review' AND GWT.TO_STATUS = 'MKTOps_Processing' AND GD.TRACKING_STATUS = 'Failed'
        THEN 'Parcel failed to deliver'

        WHEN GWT.FROM_STATUS = 'MKTOps_Processing' AND GWT.TO_STATUS = 'KAM_Proof' AND GD.GIFT_FEEDBACK IS NULL
        THEN 'Awaiting proof submission by KAM'

        WHEN GWT.FROM_STATUS = 'MKTOps_Processing' AND GWT.TO_STATUS = 'KAM_Proof' AND GD.GIFT_FEEDBACK IS NOT NULL
        THEN 'Proof submission uploaded by ' || GD.KAM_PROOF_NAME

        WHEN GWT.FROM_STATUS = 'KAM_Proof' AND GWT.TO_STATUS = 'SalesOps_Audit' AND GD.AUDIT_DATE IS NULL
        THEN 'Awaiting Audit'

        WHEN GWT.FROM_STATUS = 'KAM_Proof' AND GWT.TO_STATUS = 'SalesOps_Audit' AND GD.AUDIT_DATE IS NOT NULL
        THEN 'Gift Request verified by ' || GD.AUDITER_NAME ||
            CASE 
                WHEN GD.AUDIT_REMARK IS NOT NULL THEN ': ' || GD.AUDIT_REMARK
                ELSE '' 
        END

        WHEN GWT.FROM_STATUS = 'SalesOps_Audit' AND GWT.TO_STATUS = 'KAM_Proof'
        THEN 'Gift Request did not pass audit by ' || GD.AUDITER_NAME ||
            CASE 
                WHEN GD.AUDIT_REMARK IS NOT NULL THEN ': ' || GD.AUDIT_REMARK
                ELSE ''
        END

        WHEN GWT.FROM_STATUS = 'SalesOps_Audit' AND GWT.TO_STATUS = 'Completed'
        THEN 'Gift Request Completed'

        WHEN GWT.FROM_STATUS = 'KAM_Proof' AND GWT.TO_STATUS = 'MKTOps_Processing'
        THEN 'Gift reverted to MKTOps Processing due to delivery issue by ' || GD.KAM_PROOF_NAME ||
            CASE 
                WHEN GD.GIFT_FEEDBACK IS NOT NULL THEN ': ' || GD.GIFT_FEEDBACK
                ELSE '' 
        END

        ELSE NULL
    END AS REMARK

FROM MY_FLOW.PUBLIC.GIFT_WORKFLOW_TIMELINE GWT
LEFT JOIN MY_FLOW.PRESENTATION.VIEW_GIFT_DETAILS GD
ON GWT.GIFT_ID = GD.GIFT_ID
LEFT JOIN MY_FLOW.GLOBAL_CONFIG.SYS_USER_INFO U
ON GWT.CHANGED_BY = U.USER_ID;

--BULK IMPORT FEATURE BATCHES --
create or replace TABLE MY_FLOW.PUBLIC.BULK_IMPORT_BATCHES (
	BATCH_ID NUMBER(38,0) NOT NULL autoincrement start 10 increment 1 order,
	BATCH_NAME VARCHAR(200),
	UPLOADED_BY VARCHAR(100),
	TOTAL_ROWS NUMBER(38,0),
	CREATED_DATE TIMESTAMP_NTZ(9),
	COMPLETED_AT TIMESTAMP_NTZ(9),
	IS_ACTIVE BOOLEAN,
	primary key (BATCH_ID)
);

--Gift MKTOps Upload BO View (Schedule email notification for MKTOps and Admin to download this table for every 26th of the month 12:00 AM)
create or replace view MY_FLOW.PRESENTATION.VIEW_GIFT_MKTOPS_UPLOADBO(
	MEMBER_LOGIN,
	START_TIME,
	END_TIME,
	AMOUNT,
	NAME,
	DESCRIPTION,
	CATEGORY,
	CATEGORY_TITLE
) as SELECT 
    MEMBER_LOGIN,
    DATEADD('hour', 8, CREATED_DATE),
    DATEADD('hour', 8, MKT_DELIVERED_DATE),
    GIFT_COST,
    REWARD_NAME,
    GIFT_ITEM,
    CATEGORY,
    NULL AS CATEGORY_TITLE
    FROM MY_FLOW.PUBLIC.GIFT_DETAILS
    WHERE TRACKING_STATUS = 'Delivered'
    AND MKT_DELIVERED_DATE IS NOT NULL;